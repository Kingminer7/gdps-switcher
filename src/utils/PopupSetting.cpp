...
public:
    static Result<std::shared_ptr<PopupSetting2>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
...
protected:
    ...
    bool init(std::shared_ptr<PopupSetting2> setting, float width) {
        ...
        m_buttonSprite = ButtonSprite::create("Open", "goldFont.fnt", "GJ_button_04.png", .8f);
...
public:
    static PopupSetting2Node* create(std::shared_ptr<PopupSetting2> setting, float width) {
...
SettingNodeV3* PopupSetting2::createNode(float width) {
    return PopupSetting2Node::create(
        std::static_pointer_cast<PopupSetting2>(shared_from_this()),
        width
    );
}
...
$execute {
    (void)Mod::get()->registerCustomSettingType("open-recovery", &PopupSetting::parse);
    (void)Mod::get()->registerCustomSettingType("open-ignored", &PopupSetting2::parse);
}