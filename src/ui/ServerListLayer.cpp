...
bool ServerListLayer::init() {
  ...
  if (m_selectedServer.empty())
    m_selectedServer = GDPSMain::get()->m_currentServer;
  ...
  auto backSpr =
      cocos2d::CCSprite::createWithSpriteFrameName("GJ_arrow_03_001.png");
  auto backBtn = CCMenuItemSpriteExtra::create(
      backSpr, this, menu_selector(ServerListLayer::onBack));
  backBtn->setID("back-button");
  m_backMenu->addChild(backBtn);

  m_backMenu->setLayout(geode::ColumnLayout::create()
                            ->setAxisAlignment(geode::AxisAlignment::End)
                            ->setAxisReverse(true));
  this->addChildAtPosition(m_backMenu, geode::Anchor::TopLeft,
                           ccp(25.f, -25.f / 4.f), false);
  ...
  auto addSpr =
      cocos2d::CCSprite::createWithSpriteFrameName("GJ_newBtn_001.png");
  addSpr->setScale(.7f);
  ...
  auto editCircle = CircleButtonSprite::create(editSpr, CircleBaseColor::Green,
                                               CircleBaseSize::Big);
  ...
  m_bottomMenu->setLayout(geode::ColumnLayout::create()
                              ->setAxisAlignment(geode::AxisAlignment::Start)
                              ->setAxisReverse(false)
                              ->setGap(3.f));
  this->addChildAtPosition(m_bottomMenu, geode::Anchor::BottomRight,
                           ccp(-25.f, 25.f / 4.f), false);

  auto scrollBg = cocos2d::extension::CCScale9Sprite::create(
...
void ServerListLayer::onBack(cocos2d::CCObject *sender) {
  if (m_selectedServer != GDPSMain::get()->m_currentServer) {
    geode::createQuickPopup(
        "GDPS Switcher", "Switching servers requires a restart.",
        "Restart later", "Restart now", [this](auto, bool now) {
          if (now) {
            geode::utils::game::restart();
          } else {
            cocos2d::CCDirector::get()->replaceScene(
                cocos2d::CCTransitionFade::create(.5f,
                                                  MenuLayer::scene(false)));
          }
        });
  } else {
    cocos2d::CCDirector::get()->replaceScene(
        cocos2d::CCTransitionFade::create(.5f, MenuLayer::scene(false)));
  }
}
...
void ServerListLayer::onSelect(GDPSTypes::Server server) {
  ...
  for (auto node :
       CCArrayExt<ServerNode>(m_scroll->m_contentLayer->getChildren())) {
    if (!node)
      return;
...
void ServerListLayer::keyDown(enumKeyCodes code) {
  ...
  static const std::vector<enumKeyCodes> sequence = {
      enumKeyCodes::KEY_Up,   enumKeyCodes::KEY_Up,    enumKeyCodes::KEY_Down,
      enumKeyCodes::KEY_Down, enumKeyCodes::KEY_Left,  enumKeyCodes::KEY_Right,
      enumKeyCodes::KEY_Left, enumKeyCodes::KEY_Right, enumKeyCodes::KEY_B,
      enumKeyCodes::KEY_A,    enumKeyCodes::KEY_Space, enumKeyCodes::KEY_Enter};
